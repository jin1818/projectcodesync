...
BOOL CMainFrame::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		if(m_searchView.PreTranslateMessage(pMsg))
		{
			return TRUE;
		}
	}
...
LRESULT CMainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	m_paneTree.Create(m_splitter, _T("Search"));
	...
	m_view1.Create(m_tabbedChildWindow ,  rcDefault, CString("F:\\PHPnow-1.5.6\\htdocs\\book\\index.htm") , WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_HSCROLL | WS_VSCROLL, WS_EX_CLIENTEDGE);
	::SetWindowText(m_view1, _T("ËÑÎïÒ×") ) ;

	m_searchView.Create(m_paneTree) ;
	m_paneTree.SetClient(m_searchView);
...
BOOL CMainFrame::updateUrl(const char* pUrl , const char* pFrame)
{
	IWebBrowser2* ppBrowser = NULL ;
	this->m_view1.get_Browser(&ppBrowser) ;
	if(NULL == ppBrowser)
	...
	p2.bstrVal = SysAllocString(A2BSTR(pszMsg));
	p3.vt = VT_BSTR;
	p3.bstrVal = SysAllocString(A2BSTR(pFrame));
	
	ppBrowser->Navigate2(&p2,0,&p3,0,0);
	return TRUE ;
}
