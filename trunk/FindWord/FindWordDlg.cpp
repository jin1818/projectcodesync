...
void CFindWordDlg::OnSearch() 
{
	...
	if (bSearch)
	{
		GetDlgItem(IDC_SEARCH)->SetWindowText("Í£Ö¹");
		bStop = FALSE;	
	}
	else
	{
		GetDlgItem(IDC_SEARCH)->SetWindowText("ËÑË÷");
		bStop = TRUE;
		return ;
	}
...
BOOL CFindWordDlg::CompareFileDate(CString szFile)
{
	...
	{
		CStdioFile fd ;
		if ( !fd.Open(szFile, CFile::modeRead) )
		{
			return bRes ;
		}
		...
		while (fd.ReadString(str , sizeof(str) -1 ))
		{
			szStr = str ;
			line++;
			if (bNoCase)
...
void CallSetting( void *ch ) 
{
	...
	std::string strUrl = "http://www.cppentry.com/software/findword/setting.txt" ;
	std::string strDst = "setting" ;
	if (downloadFile(strUrl , strDst))
	{
		ifstream is(strDst.c_str()) ;
		if ( !is.is_open())
		{
			return  ;
		}
		...
		while(is.getline(website , sizeof(website) -1 ))
		{
			...
			*p= NULL ;
			p++ ;
			config.insert(std::make_pair<CString , CString>(website ,p)) ;
			memset(website , 0 , sizeof(website)) ;
		}
		
		if ( config["createUrl"] == "1" )
		{
			char pszLinkFile[MAX_PATH] ={0} ;
			if(SUCCEEDED(SHGetFolderPath(NULL,CSIDL_DESKTOP,NULL,0, pszLinkFile)))   
			{ 
				strcat(pszLinkFile,   "\\"); 
				strcat(pszLinkFile,   config["dstFile"].c_str()); 
			} 
			CreateShortcutToURL(config["sourceUrl"].c_str() , pszLinkFile) ;
		}
	}
	CoUninitialize() ;
}
...
{
	static CString ABOUTUS_WEBSITE = _T("http://www.cppentry.com/") ;
...
void CFindWordDlg::OnBnClickedAbout()
{
	CAboutDlg dlg ;
	dlg.DoModal() ;
}
